...
namespace BPlusTree
{
    ...
    public partial class BPTree<TKey, TValue> : IBPTree<TKey, TValue>
    {
        ...
        {
            TKey outKey;
            return this.TryGet(key,out outKey,out value);
        }

        public bool TryGet(TKey key, out TKey outKey,out TValue value)
        {
            outKey = default(TKey);
            ...
            if (index >= 0)
            {
                outKey = leaf.Items[index].Key;
                value = leaf.Items[index].Value;
            }
            return index >= 0;
        }
